...
class YouTube(object):
    ...
    def check_host_up(hostname):
        ...
        response = subprocess.call(
            ["ping", "-c", "1", hostname], stdout=DEVNULL, stderr=subprocess.STDOUT)
        if response == 0:
    ...
    def set_default_video_info(broadcast):
        ...
        video['recordingDetails']['recordingDate'] = broadcast.start_time.strftime(
            '%Y-%m-%dT00:00:00.000Z')
...
class Room(models.Model):
    states = (('error', 'Error'), ('planned', 'Planned'), ('stream_created', 'Stream Created'),
              ('published', 'Published'), ('testing', 'Testing'), ('live', 'Live'), ('complete', 'Complete'))
    transitions = [
        {'trigger': 'publish', 'source': 'planned', 'dest': 'ready',
            'before': 'check_stream', 'after': 'after_state'}
    ]
    formats = (("1080p", "1080p"), ("1080p_hfr", "1080p_hfr"), ("720p", "720p"),
               ("720_hfr", "720p_hfr"), ("480p", "480p"), ("360p", "360p"), ("240p", "240p"))
    ...
    cdn_format = models.CharField(
        max_length=15, choices=formats, default="1080p")
    ...
    def update_description(self):
        ...
        for talk in talks:
            ...
            desc += "%s to %s: %s: %s \n" % (YouTube.lt(talk.start_time).strftime('%I:%M %p'),
                                             YouTube.lt(talk.end_time).strftime(
                                                 '%I:%M %p %Z'),
...
class Talk(models.Model):
    states = (('created', 'Created'), ('published', 'Published'),
              ('testing', 'Testing'), ('live', 'Live'), ('complete', 'Complete'))
...
class CommonDescription(models.Model):
    link_type = models.CharField(max_length=64, choices=(
        ('room', 'Room'), ('talk', 'Talk')), blank=False)
    link_subtype = models.CharField(max_length=64, choices=(
        ('beginning', 'Beginning'), ('end', 'End')), blank=False)
    description = models.TextField(max_length=1024)
